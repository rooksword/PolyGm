...
function PolyGmEditorDrawGUI()
{
	...
	_edit.DefineTL(_w, global.bb_padding + (_y * i), string_width("Object"), _h);
	_edit.Draw();
	...
	_draw.DefineTL(_w, global.bb_padding + (_y * i), string_width("Draw"), _h);
	_draw.Draw();
	...
	{
		var _layer_current = global.layers[j];
		...
		var _l0 = new Button(string(layer_index) + " " + string_delete(_layer_current.name, 1, 3));
		_l0.DefineTL(_w, global.bb_padding + (_y * i), string_width(_l0.text), _h);
		_l0.Draw();
		if _l0.Pressed()
		{
		...
		_w += global.bb_padding + (_l0.x1 - _l0.x0);
	
		var _l1 = new Button(_vis ? "Hide" : "Show");
		_l1.DefineTL(_w, global.bb_padding + (_y * i), string_width(_l1.text), _h);
		_l1.Draw();
		if _l1.Pressed()
		{
			_layer_current.visible = !_layer_current.visible;
		}
		if _l1.Hover() hover_on_button = true;
		_w += global.bb_padding + (_l1.x1 - _l1.x0);
	
		var _str = string(_loc ? "Unlock" : "Lock");
		var _l2 = new Button(_str);
		_l2.DefineTL(_w, global.bb_padding + (_y * i), string_width(_l2.text), _h);
		_l2.Draw();
		if _l2.Pressed()
		{
			_layer_current.locked = !_layer_current.locked;
		}
		if _l2.Hover() hover_on_button = true;
		_w += global.bb_padding + (_l2.x1 - _l2.x0);
	
		var _l3 = new Button(ColourToName(_layer_current.colour) + ", " + string(_layer_current.alpha));
		_l3.DefineTL(_w, global.bb_padding + (_y * i), string_width(_l3.text), _h);
		_l3.Draw();
		if _l3.Pressed()
		{
		...
		i++; if _l3.Hover() hover_on_button = true;
	...
	if active draw_sprite(spr_cursor, -1, global.bb_mousex, global.bb_mousey);
}
